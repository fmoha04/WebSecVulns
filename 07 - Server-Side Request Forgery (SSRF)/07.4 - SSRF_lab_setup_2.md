...
```
```
docker exec PRO bash -c "apt update && apt install -y iproute2 net-tools iputils-ping apache2 php nano curl"
```
```
...
```
```
docker exec PRE bash -c "apt update && apt install -y iproute2 net-tools iputils-ping nano python3 curl"
```
...
```
```
docker exec ATTACKER bash -c "apt update && apt install -y iproute2 net-tools iputils-ping nano curl"
```
